...
class User(db.Model):
    ...
    avatar_id = db.Column(db.Integer, db.ForeignKey('picture.id'), nullable=True)
    create_time = db.Column(db.DateTime, default=db.func.current_timestamp)

    # 建立与 Picture 表的关系
    picture = db.relationship('Picture', backref=db.backref('user', lazy=True))
...
class Picture(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # 外键，指向用户 id
